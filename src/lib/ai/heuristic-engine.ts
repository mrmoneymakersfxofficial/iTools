...
export async function generateHeuristicResponse(msg: string): Promise<{ type: "text" | "products", text: string, products?: AIProduct[] }> {
  ...
  if (lower.match(/^(precio|precios|costo|cotizacion|informacion|info|ayuda|contacto)$/)) {
    return { type: "text", text: "¡Claro que sí! Para darte el precio o información exacta, por favor dime el nombre de la herramienta que buscas (ej. 'Taladro Milwaukee')." };
  }
