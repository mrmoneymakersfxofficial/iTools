...
export function CenterGiveawayBanner({ banner }: { banner: any }) {
  if (!banner) return null;

  return (
    <Link href={banner.ctaLink || "#"} className="crescent-full-width block relative w-full overflow-hidden rounded-lg">
      <div className="absolute inset-0 rounded-lg" style={{ background: banner.bgGradient || "linear-gradient(135deg, #0A2A44 0%, #0d3355 40%, #143d5e 100%)" }} />
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="font-impact text-white text-xl sm:text-2xl tracking-wider">{banner.heading}</span>
            </div>
            <p className="text-white/70 text-xs sm:text-sm mb-0.5">{banner.preTitle}</p>
            <p className="font-impact text-white text-base sm:text-xl leading-tight mb-2.5 sm:mb-4">{banner.prize}</p>
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-lg border border-white/15 p-3 sm:p-5 shrink-0 sm:w-56">
            <div className="flex items-start gap-2 mb-1">
              ...
              <p className="text-white/70 text-xs leading-relaxed">
                {banner.smsText} <span className="text-white font-bold">&quot;{banner.smsKeyword}&quot;</span><br /> al <span className="text-white font-bold">{banner.smsNumber}</span>.
              </p>
            </div>
          </div>
        </div>
