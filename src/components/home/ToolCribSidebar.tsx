...
function SidebarProductCard({ product }: { product: Product }) {
  const discount = product.comparePrice
    ...
    : 0;
  return (
    <Link href={`/producto/${product.slug}`} className="group flex gap-3 p-3 hover:bg-[#F5F6F8] transition-colors border-b border-[#F0F0F0] last:border-b-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-xs leading-snug text-[#333] group-hover:text-[#E35205] transition-colors line-clamp-2 mb-1">{product.name}</p>
        <div className="flex items-center gap-1 mb-0.5">
          <div className="flex items-center">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star key={star} className={`h-2.5 w-2.5 ${star <= Math.round(product.rating) ? "fill-amber-400 text-amber-400" : "fill-gray-200 text-gray-200"}`} />
            ))}
          </div>
          <span className="text-[10px] text-[#999]">({product.reviewCount})</span>
        </div>
        <div className="flex items-center gap-2">
          {product.comparePrice && <span className="text-[11px] text-[#999] line-through">{formatPrice(product.comparePrice)}</span>}
...
export function ToolCribSidebar() {
  ...
  return (
    <aside className="bg-white border border-[#E0E0E0] rounded-lg overflow-hidden" data-section="Productos de Moda">
      <div className="px-4 py-3 border-b border-[#E0E0E0]">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-bold text-[#1A1A1A] uppercase tracking-wide leading-tight">Tool Crib</h2>
            <p className="text-[10px] text-[#999] uppercase tracking-wider">of the North</p>
          </div>
      ...
      <div className="bg-[#F5F6F8] px-4 py-2 border-b border-[#E0E0E0]">
