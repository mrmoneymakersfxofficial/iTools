...
function MobileProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link href={`/producto/${product.slug}`} className="group flex-shrink-0 w-[48%] sm:w-[45%] bg-white dark:bg-[#111111] border border-[#E0E0E0] dark:border-[#333] rounded-lg overflow-hidden hover:shadow-md transition-shadow snap-start">
      ...
      <div className="p-2">
        {product.brand && <p className="text-[10px] font-semibold text-[#666] dark:text-gray-300 dark:text-gray-500 uppercase tracking-wide mb-0.5">{product.brand.name}</p>}
        <p className="text-[11px] text-[#333] dark:text-gray-200 leading-snug line-clamp-2 mb-1.5">{product.name}</p>
        <div className="flex items-center gap-1.5">
          {product.comparePrice && <span className="text-[10px] text-[#999] dark:text-gray-400 line-through">{formatPrice(product.comparePrice)}</span>}
